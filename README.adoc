...
A few tests are included for you to test the basic functionality of the microservices. If a test failure occurs, then you might have introduced a bug into the code. To run the tests, wait for all pods to be in the ready state before you proceed further. The default properties defined in the [hotspot file=1]`pom.xml` file are:
...
| [hotspot=system-node-port file=1]`system.node.port`          | The NodePort of the Kubernetes Service `system-service`, 31000 by default.
|===
