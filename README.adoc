...
* *Linux:* You must use kernel version of 5.9 or greater with the X86-64/AMD64 processor.
...
More detailed information on InstantOn application image can be found on https://openliberty.io/docs/latest/instanton.html.
...
After you are authenticated, create a repository for the instanton container image, in the private Amazon ECR registry, and take note of the repositoryUri value. It should match the `[aws_account_id].dkr.ecr.[region].amazonaws.com//instantOn/instantOn` pattern. For example: `1234567890.dkr.ecr.us-east-1.amazonaws.com/instantOn`. You will need this value when you create the `kubernetes.yaml` file to deploy the InstantOn application.
...
```
{
    "repository": {
        ...
        "repositoryArn": "arn:aws:ecr:[region]:[aws_account_id]:repository/instantonaws/system",
        "createdAt": 1553111916.0,
        "repositoryUri": "[aws_account_id].ecr.[region].amazonaws.com/instantonaws/system"
    }
}
```
...
Update the value of `image` to `[system-repository-uri]`
...
| [hotspot=system-node-por file=0]`system.node.port`          | The NodePort of the Kubernetes Service `system-service`, 31000 by default.
|===
