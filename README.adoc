...
:page-description: Learn how to speed up your containers using Open Liberty InstantOn and improve the CPU and memory usage using IBM Semeru Cloud Compiler on Amazon Web Services (AWS).
...
:page-seo-title: Speeding up startup time using Open Liberty InstantOn and improving CPU and memory usage by enabling IBM Semeru Cloud Compiler (SCC) in your deployment to Amazon Elastic Container Service for Kubernetes (EKS) on Amazon Web Services (AWS).
:page-seo-description: A getting started tutorial with examples on how to containerize a Java microservice with Open Liberty InstantOn feature to speed up the startup time. To improve the CPU and memory usage, deploy your application by enabling IBM Semeru Cloud Compiler (SCC) in your deploying to Amazon Elastic Kubernetes Service (EKS) on Amazon Web Services (AWS) by using Amazon Elastic Container Registry (ECR) as a private container registry.
...
Explore how to speed up your containers by using Open Liberty InstantOn and improve the CPU and memory usage by enabling IBM Semeru Cloud Compiler in your deployment to Amazon Elastic Container Service for Kubernetes (EKS) on Amazon Web Services (AWS). 
...
You will learn how to containize a microservice with Open Liberty InstantOn and deploy the container to a Kubernetes cluster on Amazon Elastic Container Service for Kubernetes (EKS).
...
The IBM Semeru Cloud Compiler decouples the JIT compiler from the JVM to prevent your Java application from suffering possible negative effects due to CPU and memory consumption caused by JIT compilation in the JVM.
