...
Create a repository for the instanton container image, in the private Amazon ECR registry, and take note of the repositoryUri value. It should match the `<aws_account_id>.dkr.ecr.<region>.amazonaws.com/instantOn` pattern. For example: `1234567890.dkr.ecr.us-east-1.amazonaws.com/instantOn`. You will need this value when you create the `deployment.yaml` file to deploy the InstantOn application.

[role=command]
```
aws ecr create-repository --repository-name instantOn
```
...
[role=command]
```
sudo podman tag dev.local/liberty-app-instanton <aws_account_id>.dkr.ecr.<region>.amazonaws.com/instantOn

sudo podman push <aws_account_id>.dkr.ecr.<region>.amazonaws.com/instantOn
```
