...
* *Linux:* You need to use kernel version of 5.9 or greater with the X86-64/AMD64 processor.
...
This command builds a target/guide-instantOn-draft-system.war archive. We can now include this WAR in a container image that uses the InstantOn feature.
...
```
{
    "repository": {
        ...
        "repositoryUri": "[aws_account_id].dkr.ecr.[region].amazonaws.com/instantonaws/system"
    }
}
```
